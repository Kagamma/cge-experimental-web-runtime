...
var
  ...
  fallthrough: boolean = false; { fallthough? }
  cases: boolean = true;
{ fixed code blocks }
var
  bl, bd : cardinal;
...
{ decompress an inflated block }
  ...
  {$endif}
    ...
    if (s.mode = BTREE) or fallthrough then
      begin
        fallthrough := false;
    ...
    if (s.mode = DTREE) or fallthrough then
      begin
        fallthrough := false;
        while TRUE do
        begin
    ...
    if (s.mode = CODES) or fallthrough then
      begin
        fallthrough := false;
        { update pointers }
        s.bitb := b;
    ...
    if (s.mode = DRY) or fallthrough then
      begin
        fallthrough := false;
        {FLUSH}
        s.write := q;
    ...
    if (s.mode = BLKDONE) or fallthrough then
      begin
        fallthrough := false;
        r := Z_STREAM_END;
        { update pointers and return }
    ...
    if (s.mode <> BLKBAD) and (s.mode <> BLKDONE) and (s.mode <> DRY)
      and (s.mode <> CODES) and (s.mode <> DTREE) and (s.mode <> BTREE)
      and (s.mode <> TABLE) and (s.mode <> STORED) and (s.mode <> LENS)
      and (s.mode <> ZTYPE) then
    begin
      r := Z_STREAM_ERROR;
      { update pointers and return }
      s.bitb := b;
